...
import { Home, Store, TrendingDown, CalendarOff, AlertCircle, CheckCircle2 } from 'lucide-react'
...
function getDaysVacant(property: Property, tenant: Tenant | null): number {
  if (tenant) return 0
  // Days since last update or creation if no tenant
    ...
    <div className="space-y-6 animate-fade-in-up">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="card p-4">
          ...
          <p className="text-2xl font-bold text-amber-600">{formatCurrency(totalRentLost)}</p>
      ...
      {loading ? (
        <div className="space-y-3">
          {[...Array(5)].map((_, i) => <div key={i} className="h-16 bg-stone-200 animate-pulse rounded-xl" />)}
        </div>
      ) : (
        <div className="card overflow-hidden">
          ...
          {filtered.length === 0 && (
            <div className="py-12 text-center text-stone-400 text-sm">No properties match this filter.</div>
          )}
        </div>
      )}
    </div>
